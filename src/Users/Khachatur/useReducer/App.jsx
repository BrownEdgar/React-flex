...
export default function App() {
  ...
  const showPosts = () => { 
    ...
    .then((res) => res.data)
    .then((data) => {
      dispatch({ type: "show-posts", payload: { data: data } });
    });
 }

 const addDev = () => {
  dispatch({ type: "add-dev", payload: { inputValue: inputValue }});
  setInputValue("")
 }
  ...
  return (
    <div className='Container'>
      <div className='Container__items'>
        ...
        <button onClick={showPosts} disabled={disableButton}>Show users</button>
        <input
        ...
        <button onClick={() => dispatch({ type: "sort"})}>Sort Array</button>
        <button onClick={() => dispatch({ type: "change-id"})}>Change ID</button>
        ...
        state.posts.map((post) => {
          return(
            <div className="Container__posts-item" key={post.id}>
              ...
              onClick={() => dispatch({ type: "delete-post", payload: `${post.id}`})}>
              <AiFillCloseSquare/>
              </button>
